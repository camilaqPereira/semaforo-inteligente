...
setup:
	...
	mov r1, #07h		; Initializing semaphore counter at 7d
	...
	mov ie, #10000001b	; Enable interrupts
...
main:
	...
	mov a, r0
	cpl a
	mov p2, a			; Update led values in P2
...
t0config:
	; Configuring timer0 -> semaphore counter
	mov tl0, #low(65535 - 50)		; Configuring initial value for timer0
	mov th0, #high(65535 - 50)		; Configuring initial value for timer0
	setb tr0				; Enable timer0
	jnb tf0, $				; Loop until timer fires

	clr tr0
	clr tf0
	dec r1					; Decrements semaphore counter
	...
	mov r1, #07h			; Update semaphore counter to 7
...
rotatestatus:
	...
	mov r1, #0ah			; Update semaphore counter to 10d
